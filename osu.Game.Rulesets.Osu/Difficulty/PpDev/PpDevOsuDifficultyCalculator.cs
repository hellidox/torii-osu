...
namespace osu.Game.Rulesets.Osu.Difficulty.PpDev
{
    public class PpDevOsuDifficultyCalculator : DifficultyCalculator
    {
        public override int Version => 20251020;

        public PpDevOsuDifficultyCalculator(IRulesetInfo ruleset, IWorkingBeatmap beatmap)
        ...
        protected override DifficultyAttributes CreateDifficultyAttributes(IBeatmap beatmap, Mod[] mods, Skill[] skills, double clockRate)
        {
            ...
            double sliderFactor = aimDifficultyValue > 0
                ? PpDevOsuRatingCalculator.CalculateDifficultyRating(aimNoSlidersDifficultyValue) / PpDevOsuRatingCalculator.CalculateDifficultyRating(aimDifficultyValue)
                : 1;

            var osuRatingCalculator = new PpDevOsuRatingCalculator(mods, totalHits, overallDifficulty);
            ...
            double baseAimPerformance = PpDevOsuPerformanceCalculator.DifficultyToPerformance(aimRating);
            ...
            double basePerformance = DifficultyCalculationUtils.Norm(PpDevOsuPerformanceCalculator.PERFORMANCE_NORM_EXPONENT, baseAimPerformance, baseSpeedPerformance, baseCognitionPerformance);
        ...
        public static double SumCognitionDifficulty(double reading, double flashlight)
        {
            ...
            return DifficultyCalculationUtils.Norm(PpDevOsuPerformanceCalculator.PERFORMANCE_NORM_EXPONENT, reading, flashlight * Math.Clamp(flashlight / reading, 0.25, 1.0));
        }

        private double calculateStarRating(double basePerformance)
        {
            return Math.Cbrt(basePerformance * PpDevOsuPerformanceCalculator.PERFORMANCE_BASE_MULTIPLIER);
        }
